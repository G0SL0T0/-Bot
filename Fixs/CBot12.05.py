...
#async def on_message(message):
    ...
    # Проверяем баланс пользователя и добавляем Жрун за каждое сообщение
    #balances = check_balance_file(message.author.id)
    #user_balance = balances[str(message.author.id)]
    #user_balance['balance'] += 1  # Добавляем 1 Жрун за каждое сообщение
    #with open('Jrun_balance.json', 'w') as f:
        #json.dump(balances, f, indent=4)
    #await message.channel.send(f'{message.author.mention}, вы получили 1 Жрун за сообщение!')
