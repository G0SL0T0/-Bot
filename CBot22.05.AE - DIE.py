...
class MenuButton(disnake.ui.Button):
    def __init__(self, label, style, custom_id, view):
        super().__init__(label=label, style=style, custom_id=custom_id)
        self.view = view

    async def callback(self, interaction):
        await interaction.response.send_message(f"Выбран пункт: {self.label}", ephemeral=True)
...
class MenuView(disnake.ui.View):
    ...
    def get_embed(self):
        if self.children[0].disabled:
            return disnake.Embed(title="Меню", description="Выберите пункт:", color=disnake.Color.blue())
        else:
            return disnake.Embed(title="Меню 2", description="Выберите пункт:", color=disnake.Color.red())

    @disnake.ui.button(label="Пункт 1", style=disnake.ButtonStyle.blue, custom_id="punkt1")
    async def button1_callback(self, button, interaction):
        button.disabled = True
        button2 = self.get_child("punkt2")
        button2.disabled = False
        await self.update_message()

    @disnake.ui.button(label="Пункт 2", style=disnake.ButtonStyle.blue, custom_id="punkt2", disabled=True)
    async def button2_callback(self, button, interaction):
        button.disabled = True
        button1 = self.get_child("punkt1")
        button1.disabled = False
        await self.update_message()

    def get_child(self, custom_id):
...
############################################################################################################
############# обработчик EMBED сообщений с кнопками ####################
...
def setup(bot):
    ...
    async def menu(interaction: disnake.CommandInteraction):
        view = MenuView(bot)
